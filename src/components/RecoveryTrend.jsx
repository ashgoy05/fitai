...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{ background: '#141414', border: `1px solid ${color}`, borderRadius: '8px', padding: '10px 14px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          {plan.weekly_insight && (
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--yellow)', letterSpacing: '2px', marginBottom: '8px' }}>
                📊 THIS WEEK
              </div>
              <p style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: 1.7 }}>{plan.weekly_insight}</p>
            </div>
          )}
          {plan.monthly_insight && (
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--green)', letterSpacing: '2px', marginBottom: '8px' }}>
                📈 THIS MONTH
              </div>
              <p style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: 1.7 }}>{plan.monthly_insight}</p>
            </div>
          )}
        </div>
      )}
        ...
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="score" stroke="#00ff87" strokeWidth={2} fill="url(#recoveryGrad)" dot={{ fill: '#00ff87', r: 4 }} activeDot={{ r: 6 }} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
        {[
          ...
          <div key={s.label} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px', textAlign: 'center' }}>
